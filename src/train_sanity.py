# ---- resilient imports: work in both "module" and "script" modes
...
ROOT_DIR = os.path.dirname(THIS_DIR)        # project root (contains "src", "configs")

# Ensure both the project root and src/ are on sys.path
for p in (ROOT_DIR, THIS_DIR):
    if p not in sys.path:
        sys.path.insert(0, p)

try:
    # Try package-style (when run as: python -m src.train_sanity)
    from src.utils import load_config, set_seed, pick_device
    from src.wrappers import PreprocessObs, FrameStack, ClipReward
    from src.model import DQN
    from src.replay import ReplayBuffer
    from src.policies import epsilon_greedy, softmax_policy
except Exception:
    # Fallback to intra-src imports (when run as: python src/train_sanity.py)
...
def main():
    ...
    optim_ = optim.Adam(online.parameters(), lr=cfg["learning_rate"])
    ...
    for frame in range(1, total_frames + 1):
        ...
        epsilon = linear_epsilon(frame, cfg["epsilon_start"], cfg["epsilon_end"], cfg["epsilon_decay_frames"])
        action = epsilon_greedy(q.squeeze(0), epsilon)
        ...
        if done:
            obs, info = env.reset()
            print(f"[episode] return={ep_return:.1f} len={ep_len}")
            ep_return, ep_len = 0.0, 0
        ...
        if frame > start_learn and frame % train_freq == 0 and rb.can_sample(batch_size):
            ...
            if frame % 1000 == 0:
                print(f"[train] frame={frame} loss={loss.item():.4f}")
